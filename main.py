    ...
    async def on_candle_close(self, df):
        ...
        try:
            ...
            if can_enter and signal != "HOLD":
                ...
                # If total entries count including DCA are >= 4, next DCA trade amount is 50% of trade amount
                size_multiplier = 0.5 if current_entries_count >= 4 else 1.0
                ...
                if qty > 0:
                    ...
                    if size_multiplier < 1.0:
                        logger.info(f"[{self.network.upper()} {self.symbol}] DCA entries >= 4 ({current_entries_count}x). Reduced trade amount by 50% -> Qty: {qty}")
